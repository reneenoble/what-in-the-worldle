    ...
    def play(self, solver):
        ...
        for i in range(6):
            ...
            if guess == self.answer:
                print("You did it! 🎉🎉🎉")
                # break
                return (True, i+1, view)
        else:
            print("bad luck!")
            print(f"The answer was { self.answer.upper() }")
            return (False, None, view)
...
class Solver():
    ...
    def guess(self, guesses, results):
        if len(guesses) == 0:
            ...
            # print(self.score_possibilites(allow_repeat_letters=True)[:20])
            return self.score_possibilites(allow_repeat_letters=True, common_words_only=True)[0][0]
    ...
    def score_possibilites(self, allow_repeat_letters=False, common_words_only=False):
        ...
        for word in self.possible_words:
            ...
            if common_words_only:
                score_count[word] = score_count[word] * NLP_SCORES[word]
...
NLP_SCORES = nlp_data.load_scores("scores_simpsonsmax.csv")
print(list(NLP_SCORES.items())[:10])

answers = ['acute', 'ferry', 'aside', 'tapir', 'troll', 'unify', 'rebus', 'boost', 'truss', 'siege', 'tiger', 'banal', 'slump', 'crank', 'gorge', 'drink', 'favor', 'abbey', 'tangy', 'panic', 'solar', 'shire', 'proxy', 'point', 'robot', 'prick', 'wince', 'crimp', 'knoll', 'sugar', 'whack', 'mount', 'perky', 'could', 'wrung', 'light', 'those', 'moist', 'shard', 'pleat', 'aloft', 'skill', 'elder', 'frame', 'humor', 'pause', 'ulcer', 'ultra', 'robin', 'cynic', 'aroma', 'caulk', 'shake', 'dodge', 'swill', 'tacit', 'other', 'thorn', 'trove', 'bloke', 'vivid', 'spill', 'chant', 'choke', 'rupee', 'nasty', 'mourn', 'ahead', 'brine', 'cloth', 'hoard', 'sweet', 'month', 'lapse', 'watch', 'today', 'focus', 'smelt', 'tease', 'cater', 'movie', 'saute', 'allow', 'renew', "their"]
...
for answer in answers:
    ...
    solver = Solver()
    win, turn, view = game.play(solver)
...
print("#################")
for a, w, v in zip(answers, wins, views):
    if not w:
        print("correct: ", a)
        print(v)
        print()